...
public:
  ...
  bool is_sorted() const {
      for (size_t i = 0; i < total_count(); i++) {
        if (i < light_count() ) { // light_count = total_count / 2
          if (_colors[i] == DISK_DARK) {
            return false;
          } 
        } else { // Check if (i > light_count)
            if (_colors[i] == DISK_LIGHT) {
              return false;
            }
          }
...
sorted_disks sort_alternate(const disk_state& before) {
  ...
  for (int i = 0; i < (state.total_count() / 2); i++) {
    for (int j = i; j < state.total_count() - i; j += 2) {
...
sorted_disks sort_lawnmower(const disk_state& before) {
  int numOfSwap = 0;
  disk_state state(before);
  for (int i = 1; i < (state.total_count() / 2); i++) {
        for (int j = i; j < (state.total_count() - i - 1); j++) {
            if (state.get(j) > state.get(j + 1)) {
              state.swap(j);
              numOfSwap++;
            }
        }
        for (int j = (state.total_count() - i - 2); j > i + 1; j--) {
            if (state.get(j) < state.get(j-1)) {
              state.swap(j-1);
              numOfSwap++;
            }
